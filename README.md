...
or `./stop.sh tp4`; `./start.sh stop` still only targets TP=2.

```bash
# edit WORKER2_IP / WORKER3_IP / CX7 pins in .env.tp4
./start-tp4.sh
./start-tp4.sh stop
./stop.sh tp4             # equivalent lifecycle dispatch through the parent stopper
...
| `tests/test_tp4_launch_chain.py` | TP=4 direct target/DFlash2 asset gates and parent `stop.sh` TP=4 dispatch (CPU-only) |
| `start.sh` / `stop.sh` / `download.sh` | 2-node launch; Hub fetch on the head only. `./stop.sh` dispatches TP=2/3/4 (`tp2` / `tp3` / `tp4` / `all`) |
