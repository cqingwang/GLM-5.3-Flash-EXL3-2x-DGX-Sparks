"""Apply the pinned TP4 sparse-MLA attention slice.

The slice is opt-in through ``VLLM_SM120_SPARSE_MLA_SLICE_TOKENS=64``.
The source hash guard prevents silently applying this workaround to a
different FlashInfer backend revision.
"""
...
def patched_source(original: str) -> str:
    ...
    result = result.replace(
        ...
        CONSTANTS + "def _kv_scale_format_for_model",
        1,
    )
        ...
        json.dumps(
            {
                ...
                "slice_tokens": os.environ.get("VLLM_SM120_SPARSE_MLA_SLICE_TOKENS", "0"),
                "status": "PASS",
            }
        )
    )
